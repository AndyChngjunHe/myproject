...
class SimpleTrainer(TrainerBase):
    ...
    def test_zs(self, split=None):
        ...
        for i, output in enumerate(modified_outputs):
            # 确保将张量从CUDA转移到CPU，并转换为NumPy数组
            temp_df = pd.DataFrame(output.cpu().numpy())

            # 给每列添加一个前缀，以防止列名冲突
            temp_df = temp_df.add_prefix(f"Batch_{i}_")

            # 将转换后的DataFrame添加到大的DataFrame中
            df = pd.concat([df, temp_df], axis=1)
        #csv文件里，每个batch一行代表该点云对这几个类的概率
        df.to_csv("data/modified_outputs.csv", index=False)
